...
namespace OnCourse.Domo.DataSets
{
    public class DataSetClient : IDataSetClient
    {
        ...
        public async Task<DataSet> CreateDataSetAsync(DataSet dataSet)
        {
            ...
            var content = new StringContent(JsonSerializer.Serialize(dataSet, _serializerOptions), Encoding.UTF8, "application/json");
        ...
        {
            var uri = $"v1/datasets/query/execute/{dataSetId}";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");
            ...
            var content = new StringContent(JsonSerializer.Serialize(formContent, _serializerOptions), Encoding.UTF8, "application/json");
            var response = await _domoHttpClient.Client.PostAsync(uri, content);
            var stringResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IEnumerable<DataSet>> ListDataSetsAsync(string sort, long offset = 0, long limit = 50)
        {
            ...
            var uri = $"v1/datasets?limit={limit}&offset={offset}&sort={sort}";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");
        ...
        {
            var uri = $"v1/datasets/{dataSetId}/data";
        ...
        {
            var uri = $"v1/datasets/{dataSetId}/data?includeHeader={includeHeader}&filename={fileName}";
            _domoHttpClient.SetAcceptRequestHeaders("text/csv");
        ...
        {
            var uri = $"v1/datasets/{dataSetId}/policies/{pdpId}";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");
        ...
        {
            var uri = $"v1/datasets/{dataSetId}/policies";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");

            var content = new StringContent(JsonSerializer.Serialize(policy, _serializerOptions), Encoding.UTF8, "application/json");
            var response = await _domoHttpClient.Client.PostAsync(uri, content);
            var stringResponse = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<Policy>(stringResponse, _serializerOptions);
        }
