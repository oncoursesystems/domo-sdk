...
namespace OnCourse.Domo.Users
{
    public class UserClient : IUserClient
    {
        ...
        {
            var userId = $"v1/users?sendInvite={sendInvite}";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");

            var content = new StringContent(JsonSerializer.Serialize(user, _serializerOptions), Encoding.UTF8, "application/json");
        ...
        public async Task<IEnumerable<User>> ListUsersAsync(long offset, long limit = 50)
        {
            ...
            var userUri = $"v1/users?limit={limit}&offset={offset}";
            _domoHttpClient.SetAcceptRequestHeaders("application/json");
